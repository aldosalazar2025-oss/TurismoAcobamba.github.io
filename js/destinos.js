  ...
  grid.innerHTML=list.map(d=>{
    ...
    return `<article class="destination-card v6-card">
      <div class="destination-img">
        ${cover?`<img src="${cover}" alt="${safe(d.nombre)}" loading="lazy" style="object-position:${d.fotoPos||"center"};object-fit:${d.fotoFit||"cover"}" onerror="window.__imgFallback(this,'${d.emoji}')">`:`<span>${d.emoji}</span>`}
        <span class="difficulty">${safe(d.dificultad)}</span>
