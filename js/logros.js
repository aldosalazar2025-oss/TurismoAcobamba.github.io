...
async function render(){
 ...
 let completed=[];
 if(auth.currentUser){try{const s=await getDocs(collection(db,"usuarios",auth.currentUser.uid,"logros"));completed=s.docs.map(x=>x.id)}catch{}}
 grid.innerHTML=badges.map(b=>{const ok=completed.includes(b.id);return `<article class="badge-card ${ok?"":"locked"}"><div class="badge-icon">${ok?b.icon:"🔒"}</div><h3>${ok?b.name:"Insignia bloqueada"}</h3><p>${ok?b.desc:"Completa el destino correspondiente para desbloquearla."}</p>${ok?'<span class="tag">✓ Obtenida</span>':""}</article>`}).join("");
}
render();
